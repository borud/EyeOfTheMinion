...
// The pins for the flashing lights
#define LIGHTS_RUN_PIN       4
#define LIGHTS_STROBE_PIN    5
...
void setup() {
    ...
    pinMode(LIGHTS_RUN_PIN, OUTPUT);
    pinMode(LIGHTS_STROBE_PIN, OUTPUT);
    
    digitalWrite(ENABLE_PIN, LOW);
    digitalWrite(LIGHTS_RUN_PIN, LOW);
    digitalWrite(LIGHTS_STROBE_PIN, LOW);
...
void lights_strobe() {
    digitalWrite(LIGHTS_STROBE_PIN, HIGH);
    digitalWrite(LIGHTS_RUN_PIN, LOW);
    light_mode = strobe;
}
...
void lights_run() {
    digitalWrite(LIGHTS_STROBE_PIN, LOW);    
    digitalWrite(LIGHTS_RUN_PIN, HIGH);
    light_mode = run;
}
...
void show_help() {
    ...
    Serial.println(F("101 r (run lights)   - running pattern on lights"));
    Serial.println(F("101 f (strobe lights) - flash pattern on lights"));
    Serial.println(F("101 o (lights        - flash pattern on lights"));
}
...
void show_status() {
    ...
    switch(light_mode) {
    case off:
        Serial.println(F("OFF light"));
        break;

    case run:
        Serial.println(F("RUN light"));
        break;

    case strobe:
        Serial.println(F("STROBE light"));
        break;
...
char* parse_command(char* s) {
    ...
    switch (s[0]) {
        ...
        case 'r':
            lights_run();
            return "205 OK - RUN LIGHTS ON";
            
        case 'f':
            lights_strobe();
            return "206 OK - STROBE LIGHTS ON";

        case 'o':
            lights_off();
            return "207 OK - LIGHTS OFF";
